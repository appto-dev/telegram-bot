...
## 20.1 Ключи конфига (`config/telegram-bot.php`)
...
| `telegram:webhook-secret` | Сгенерировать случайную строку для `TELEGRAM_BOT_WEBHOOK_SECRET` (см. [2.2](02-installation.md#22-переменные-окружения)). То же самое — `Appto\TelegramBot\Support\SecretGenerator::generate()` из кода |
...
- **Событие (Event)** — Laravel-событие, которое пакет диспатчит в конкретный момент (пришёл
  апдейт, выполнен вызов Bot API, диалог стартовал/завершился), чтобы код приложения мог
  среагировать через `Event::listen()`, не трогая сам пакет (см. [19. События](19-events.md)).
